...
class MicroWebSrv2:
    ...
    _DEFAULT_PAGES = ["index.html", "index.htm", "default.html", "default.htm"]
    ...
    WARNING = 0x02
    ERROR = 0x03
    MSG_TYPE_STR = {DEBUG: "DEBUG", INFO: "INFO", WARNING: "WARNING", ERROR: "ERROR"}
    ...
    def StartInPool(self, asyncSocketsPool):
        ...
        try:
            xBufSlots = XBufferSlots(
                slotsCount=self._slotsCount,
        ...
        try:
            self._xasSrv = XAsyncTCPServer.Create(
                asyncSocketsPool=asyncSocketsPool,
    ...
    def _onSrvClientAccepted(self, xAsyncTCPServer, xAsyncTCPClient):
        if self._sslContext:
            try:
                xAsyncTCPClient.StartSSLContext(
                    sslContext=self._sslContext, serverSide=True
                )
            except:
                self.Log(
                    "SSL connection failed from %s:%s." % xAsyncTCPClient.CliAddr,
                    MicroWebSrv2.DEBUG,
                )
    ...
    def IsRunning(self):
        return (
            self._xasPool is not None
            and self._xasPool.WaitEventsProcessing
            and self._xasSrv is not None
        )
