...
class XAsyncSocketsPool:
            ...
            try:
                ...
                for socketsList in ex, wr, rd:
                    for socket in socketsList:
                        ...
                        if asyncSocket and self._socketListAdd(
                            socket, self._handlingList
                        ):
                ...
                if sec > timeSec + self._CHECK_SEC_INTERVAL:
                    ...
                    for asyncSocket in list(self._asyncSockets.values()):
                        if (
                            asyncSocket.ExpireTimeSec
                            and timeSec > asyncSocket.ExpireTimeSec
                        ):
    ...
    def AddAsyncSocket(self, asyncSocket):
        try:
            ...
            raise XAsyncSocketsPoolException(
                'AddAsyncSocket : "asyncSocket" is incorrect.'
            )
    ...
    def RemoveAsyncSocket(self, asyncSocket):
        try:
            ...
            raise XAsyncSocketsPoolException(
                'RemoveAsyncSocket : "asyncSocket" is incorrect.'
            )
    ...
    def NotifyNextReadyForReading(self, asyncSocket, notify):
        try:
            ...
            raise XAsyncSocketsPoolException(
                'NotifyNextReadyForReading : "asyncSocket" is incorrect.'
            )
        if notify:
    ...
    def NotifyNextReadyForWriting(self, asyncSocket, notify):
        try:
            ...
            raise XAsyncSocketsPoolException(
                'NotifyNextReadyForWriting : "asyncSocket" is incorrect.'
            )
        if notify:
    ...
    def AsyncWaitEvents(self, threadsCount=0):
        ...
        if threadsCount > 0:
            try:
                ...
                self._processing = False
                raise XAsyncSocketsPoolException(
                    "AsyncWaitEvents : Fatal error to create new threads..."
                )
        else:
...
class XAsyncTCPServer(XAsyncSocket):
    ...
    def Create(asyncSocketsPool, srvAddr, srvBacklog=256, bufSlots=None):
        try:
            ...
            raise XAsyncTCPServerException(
                "Create : Cannot open socket (no enought memory)."
            )
        try:
        ...
        xAsyncTCPServer = XAsyncTCPServer(
            asyncSocketsPool, srvSocket, srvAddr, bufSlots
        )
    ...
    def __init__(self, asyncSocketsPool, srvSocket, srvAddr, bufSlots):
        try:
            ...
            raise XAsyncTCPServerException(
                "Error to creating XAsyncTCPServer, arguments are incorrects."
            )
    ...
    def OnReadyForReading(self):
        ...
        if not recvBufSlot or not sendBufSlot or not self._onClientAccepted:
            ...
            cliSocket.close()
            return
        asyncTCPCli = XAsyncTCPClient(
            self._asyncSocketsPool,
            cliSocket,
            self._srvAddr,
            cliAddr,
            recvBufSlot,
        ...
        try:
            ...
            asyncTCPCli._close()
            raise XAsyncTCPServerException(
                'Error when handling the "OnClientAccepted" event : %s' % ex
            )
...
class XAsyncTCPClient(XAsyncSocket):
    ...
    def Create(
        ...
        asyncTCPCli = XAsyncTCPClient(
            asyncSocketsPool, cliSocket, srvAddr, None, recvBufSlot, sendBufSlot
        )
        ok = False
        try:
            if connectAsync and hasattr(cliSocket, "connect_ex"):
                ...
                try:
                    addr = socket.getaddrinfo(srvAddr[0], srvAddr[1], socket.AF_INET)
                except:
                    addr = socket.getaddrinfo(srvAddr[0], srvAddr[1])
                addr = addr[0][-1]
                if connectAsync:
                    asyncTCPCli._setExpireTimeout(connectTimeout)
                else:
    ...
    def __init__(
        self, asyncSocketsPool, cliSocket, srvAddr, cliAddr, recvBufSlot, sendBufSlot
    ):
        try:
        ...
        if not self._socketOpened:
                ...
                if self._socket.getsockopt(socket.SOL_SOCKET, socket.SO_ERROR):
                    ...
                    if self._onFailsToConnect:
                        try:
                            ...
                            raise XAsyncTCPClientException(
                                'Error when handling the "OnFailsToConnect" event : %s'
                                % ex
                            )
                    return
            ...
            if self._onConnected:
                try:
                    ...
                    raise XAsyncTCPClientException(
                        'Error when handling the "OnConnected" event : %s' % ex
                    )
        if self._wrBufView:
            try:
    ...
    def StartSSL(
        self, keyfile=None, certfile=None, server_side=False, cert_reqs=0, ca_certs=None
    ):
        if not hasattr(ssl, "SSLContext"):
            raise XAsyncTCPClientException(
                "StartSSL : This SSL implementation is not supported."
            )
        if self.IsSSL:
    ...
    def StartSSLContext(self, sslContext, serverSide=False):
        if not hasattr(ssl, "SSLContext"):
            raise XAsyncTCPClientException(
                "StartSSLContext : This SSL implementation is not supported."
            )
        if not isinstance(sslContext, ssl.SSLContext):
            raise XAsyncTCPClientException(
                'StartSSLContext : "sslContext" is incorrect.'
            )
        if self.IsSSL:
        ...
        try:
            self._asyncSocketsPool.NotifyNextReadyForWriting(self, False)
            self._asyncSocketsPool.NotifyNextReadyForReading(self, False)
            self._socket = sslContext.wrap_socket(
                self._socket, server_side=serverSide, do_handshake_on_connect=False
            )
        except Exception as ex:
    ...
    @property
    def IsSSL(self):
        return hasattr(ssl, "SSLContext") and isinstance(self._socket, ssl.SSLSocket)
...
class XAsyncUDPDatagram(XAsyncSocket):
    ...
    def Create(asyncSocketsPool, localAddr=None, recvBufLen=4096, broadcast=False):
        try:
            ...
            raise XAsyncUDPDatagramException(
                "Create : Cannot open socket (no enought memory)."
            )
        if broadcast:
        ...
        if openRecv:
            try:
                ...
                raise XAsyncUDPDatagramException(
                    "Create : Error to binding the UDP Datagram local address."
                )
            try:
                size = max(256, recvBufLen)
    ...
    def __init__(self, asyncSocketsPool, udpSocket, recvBufSlot):
        try:
            ...
            raise XAsyncUDPDatagramException(
                "Error to creating XAsyncUDPDatagram, arguments are incorrects."
            )
            ...
            try:
                ...
                if self._onFailsToSend:
                    try:
                        ...
                        raise XAsyncUDPDatagramException(
                            'Error when handling the "OnFailsToSend" event : %s' % ex
                        )
            if not self._wrDgramFiFo.Empty:
                return
        self._asyncSocketsPool.NotifyNextReadyForWriting(self, False)
    ...
    def AsyncSendDatagram(
        self, datagram, remoteAddr, onDataSent=None, onDataSentArg=None
    ):
        if self._socket:
            try:
                ...
                pass
            raise XAsyncUDPDatagramException(
                "AsyncSendDatagram : Arguments are incorrects."
            )
        return False
